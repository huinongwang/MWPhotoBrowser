Pod::Spec.new do |s|
  s.name = 'MWPhotoBrowser'
  s.version = '2.1.2.1'
  s.author = { 'huihongwang' => 'iosdev@cnhnkj.com' }
  s.license = 'MIT'
  s.homepage = 'https://github.com/huinongwang/MWPhotoBrowser'
  s.source = { :git => 'https://github.com/huinongwang/MWPhotoBrowser.git', :tag => s.version }
  s.summary = '基于MWPhotoBrowser的修正版'

  s.requires_arc = true
  s.platform = :ios, '8.0'
  s.source_files  = 'PodSourceFiles/Classes/*'
  s.resource = 'PodSourceFiles/Assets/MWPhotoBrowser.bundle'
  s.public_header_files = 'PodSourceFiles/Classes/*.h'

  s.frameworks = 'ImageIO', 'QuartzCore', 'AssetsLibrary', 'MediaPlayer'
  s.weak_frameworks = 'Photos'

  s.dependency 'MBProgressHUD', '~> 1.2'
  s.dependency 'DACircularProgress', '~> 2.3'
  s.dependency 'SDWebImage', '~> 5.10'
end
